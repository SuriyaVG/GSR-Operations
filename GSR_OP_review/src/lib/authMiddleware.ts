    ...
    try {
      ...
      if (requireAuth) {
        ...
        if (customCheck && !customCheck(currentUser, context)) {
          ...
          // Log permission denied for custom check failure
  ...
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      if (!AuthorizationService.hasRole(currentUser, roles)) {
        ...
        SecurityAuditService.logRoleEscalationAttempt(
          currentUser.id,
          currentUser.role,
          roles.join(' or '),
          propertyKey
        ).catch(err => console.error('Failed to log role escalation attempt:', err));
