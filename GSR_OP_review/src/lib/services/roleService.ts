...
export class RoleService {
  ...
  static async changeUserRole(
    ...
    try {
      ...
      // Log security event for role assignment change
      const { SecurityAuditService } = await import('./securityAuditService');
      await SecurityAuditService.logSecurityEvent({
        eventType: 'role_assignment_change' as any, // Will be added to enum
        userId: adminId,
        resource: 'user_role',
        action: 'update',
        userRole: admin.role,
  ...
  static async getAllUsersForManagement(adminId: string): Promise<UserManagementData[]> {
    try {
      ...
      // Use the user_management_view to get all users with joined auth data
      ...
      const managementData: UserManagementData[] = profiles.map(profile => ({
        ...
        name: profile.name,
        role: profile.role,
        designation: profile.designation, // may be undefined in view, fallback to undefined
        active: profile.active,
        last_login: profile.last_login,
        created_at: profile.created_at,
        updated_at: profile.updated_at,
        custom_settings: profile.custom_settings // may be undefined in view, fallback to undefined
      }));
