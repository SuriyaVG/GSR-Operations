...
-- Create custom types
CREATE TYPE user_role AS ENUM ('admin', 'production', 'sales_manager', 'finance', 'viewer');
CREATE TYPE customer_tier AS ENUM ('premium', 'wholesale', 'standard');
CREATE TYPE customer_channel AS ENUM ('direct', 'distributor', 'online', 'retail');
...
CREATE TYPE credit_note_status AS ENUM ('draft', 'issued', 'applied');
CREATE TYPE interaction_type AS ENUM ('call', 'email', 'whatsapp', 'meeting');
CREATE TYPE return_status AS ENUM ('pending', 'approved', 'processed', 'rejected');
CREATE TYPE transaction_type AS ENUM ('invoice', 'payment', 'credit_note', 'refund', 'adjustment');
CREATE TYPE reference_type AS ENUM ('invoice', 'payment', 'credit_note', 'order');
